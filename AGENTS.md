...
| Script Suite Bundler (`pack.ps1`) | ✅ Implemented | Automated build script `pack.ps1` bundles all `.ps1` modules, TUI components, JSON configs (`apps.json`, `printers.json`, `departments.json`), XML assets (`OfficeCustom.xml`), and custom installer scriptblocks into a single standalone, zero-dependency script: `dist/szc_setup_bundled.ps1` (~81 KB). Fixed `MyInvocation.MyCommand.Path` null error when running custom installers in memory/bundled scriptblocks. Validated clean syntax with 0 AST parse errors. Created `.gitignore` to ignore the `dist/` build directory. |
