...
| Printer implementation | ✅ Implemented | Three-mode `Install-LocalPrinter` (IPP/TCP/LPR), integrated into `Start-Deployment`, config-driven via `printers.json`. Configured real driver URLs for Brother T4500DW (`Brother MFC-T4500DW Printer`) and Epson L1800 (`EPSON L1800 Series`). Analyzed driver executables and added automated 7-Zip extraction + `pnputil.exe /add-driver` INF staging for silent unattended installation. Legacy `printers_dn.ps1` removed. On `feature/printer-install` branch. |
